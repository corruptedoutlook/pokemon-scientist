...
OaksLab_ScriptPointers:
	...
	dw_const OaksLabAideFaceUpScript,								 SCRIPT_OAKSLAB_AIDE_FACE_UP
	dw_const OaksLabAideStartsWalkingScript,				 SCRIPT_OAKSLAB_AIDE_STARTS_WALKING
	dw_const OaksLabAideGivesPokeballsScript,        SCRIPT_OAKSLAB_AIDE_GIVES_POKEBALLS
	dw_const OaksLabAideReturnScript,                SCRIPT_OAKSLAB_AIDE_RETURN
	dw_const OaksLabOakEndBattleScript, 						 SCRIPT_OAKSLAB_OAK_END_BATTLE
	dw_const OaksLabPostOakBattleScript, SCRIPT_OAKSLAB_POST_OAK_BATTLE
...
;	predef HideObject
	...
	ld [wRivalStarter], a     ; Keep STARTER1/2/3 for trainer selection
	ld a, PORYGON
	ld [wCurPartySpecies], a  ; But actually give him Porygon
	ld [wNamedObjectIndex], a
	call GetMonName
	ld a, TEXT_OAKSLAB_OAK_BE_PATIENT ; Test this
	ldh [hTextID], a
	call DisplayTextID
	ld a, OAKSLAB_RIVAL
	ldh [hSpriteIndex], a
...
OaksLabPlayerWatchRivalExitScript:
	...
	ld a, SCRIPT_OAKSLAB_AIDE_STARTS_WALKING
	ld [wOaksLabCurScript], a
	jr .done
...
OaksLabNoopScript:
	ret
OaksLabAideFaceUpScript:
	ld a, OAKSLAB_SCIENTIST1
	ldh [hSpriteIndex], a
	ld a, SPRITE_FACING_UP
	ldh [hSpriteFacingDirection], a
	jp SetSpriteFacingDirectionAndDelay
OaksLabAideStartsWalkingScript:
		; Lock controls.
	ld a, PAD_SELECT | PAD_START | PAD_CTRL_PAD
	ld [wJoyIgnore], a
		; Choose movement based on player's X coordinate.
	ld de, .LeftAideMovementPlayerX5
	ld a, [wXCoord]
	cp 4
	jr nz, .move
	ld de, .LeftAideMovementPlayerX4
.move
	ld a, OAKSLAB_SCIENTIST1
	ldh [hSpriteIndex], a
	call MoveSprite
	ld a, SCRIPT_OAKSLAB_AIDE_GIVES_POKEBALLS
	ld [wOaksLabCurScript], a
	ret
.LeftAideMovementPlayerX4
	db NPC_MOVEMENT_UP
	db NPC_MOVEMENT_RIGHT
	db NPC_MOVEMENT_RIGHT
	db NPC_MOVEMENT_UP
...
.LeftAideMovementPlayerX5
	...
	db NPC_MOVEMENT_UP
	db -1
OaksLabAideGivesPokeballsScript:
		; Wait until aide finishes walking.
	ld a,[wStatusFlags5]
	bit BIT_SCRIPTED_NPC_MOVEMENT,a
	ret nz
	call EnableAutoTextBoxDrawing
	...
	xor a
	ld [wJoyIgnore],a
	call OaksLabAideFaceUpScript
	ld a,TEXT_OAKSLAB_AIDE_POKEBALLS
	ldh [hTextID],a
	call DisplayTextID
			; Give Poké Balls.
	CheckAndSetEvent EVENT_GOT_POKEBALLS_FROM_OAK
	jr nz,.alreadyGotThem
	lb bc,POKE_BALL,5
	call GiveItem
			; Set up item name.
	ld a, POKE_BALL
	...
	call GetItemName
			; Display "<PLAYER> got @!"
	call OaksLabAideFaceUpScript
	ld a, TEXT_OAKSLAB_AIDE_RECEIVED_POKEBALLS
	ldh [hTextID], a
	call DisplayTextID
	call OaksLabAideFaceUpScript
	ld a, TEXT_OAKSLAB_AIDE_AFTER_POKEBALLS
	ldh [hTextID], a
	call DisplayTextID
.alreadyGotThem
			; Start return movement.
	ld de,.ReturnMovementPlayerX5
	ld a,[wXCoord]
	cp 4
	jr nz,.move
	ld de,.ReturnMovementPlayerX4
.move
	ld a,OAKSLAB_SCIENTIST1
	ldh [hSpriteIndex],a
	call MoveSprite
	ld a,SCRIPT_OAKSLAB_AIDE_RETURN
	ld [wOaksLabCurScript],a
	ret
.ReturnMovementPlayerX4
	db NPC_MOVEMENT_DOWN
...
OaksLab_TextPointers:
	...
	dw_const OaksLabAidePokeballsText,				    TEXT_OAKSLAB_AIDE_POKEBALLS
	dw_const OaksLabAideAfterPokeballsText,			  TEXT_OAKSLAB_AIDE_AFTER_POKEBALLS
	dw_const OaksLabAideReceivedPokeballsText,		TEXT_OAKSLAB_AIDE_RECEIVED_POKEBALLS
	dw_const OaksLabOakDiplomaText, 							TEXT_OAKSLAB_OAK_DIPLOMA
...
OaksLabOak1Text:
	...
	cp NUM_POKEMON
	jr c, .parcelLogic
	; Have all eight Gym Leader rematches been cleared?
	ld a, [wGymLeaderRematchFlags]
	cp $ff
	jr nz, .parcelLogic
	jr .challengeOak
.showDiploma
	ld a, TEXT_OAKSLAB_OAK_DIPLOMA
	ldh [hTextID], a
	call DisplayTextID
	jp TextScriptEnd
.parcelLogic
	; Already suspicious: repeat suspicious dialogue.
	CheckEvent EVENT_STOLE_OAKS_PARCEL
	jp nz, .stole_parcel
	; Parcel already successfully delivered: normal Oak dialogue.
	CheckEvent EVENT_OAK_GOT_PARCEL
	jp nz, .already_got_poke_balls
	; If the parcel was used, we still need to ask the YES/NO question.
	CheckEvent EVENT_USED_OAKS_PARCEL
	jp nz, .ask_about_parcel
	; If the parcel is physically in the bag, ask about it.
	ld b, OAKS_PARCEL
	call IsItemInBag
	jr nz, .ask_about_parcel
	; Player has not received the parcel yet: normal early-game dialogue.
	ld a, [wStatusFlags4]
...
.challengeOak
	...
	ld a, [wCurrentMenuItem]
	and a
	jr z, .acceptChallenge
	ld hl, .RefusedChallengeText
	call PrintText
	jp .done
.acceptChallenge
	ld a, OPP_PROF_OAK
...
.ask_about_parcel
	...
	and a
	jr nz, .refuse_parcel
	; YES: check if the parcel actually exists.
	ld b, OAKS_PARCEL
	call IsItemInBag
	jr z, .yes_parcel_used
	; YES + parcel in inventory.
	call OaksLabScript_RemoveParcel
	SetEvent EVENT_OAK_GOT_PARCEL
	ld hl, .ParcelThanksText
	call PrintText
	jr .done
.yes_parcel_used
	; YES + used parcel.
	SetEvent EVENT_STOLE_OAKS_PARCEL
	ld hl, .UsedParcelText
	call PrintText
	jr .done
.refuse_parcel
	; NO + inventory OR used.
	SetEvent EVENT_STOLE_OAKS_PARCEL
	ld hl, .KeepParcelText
	call PrintText
	jr .done
.stole_parcel
	ld hl, .StoleParcelText
	call PrintText
	jr .done
...
.ParcelThanksText:
	...
	para "-and in perfect"
	line "condition, too!"
	para "You handled this"
	line "exactly as I"
	cont "wanted, <PLAYER>!"
	para "Thank you!"
	done
.KeepParcelText:
	text_far _OaksLabOak1KeepParcelText
	text_end
.StoleParcelText:
	text_far _OaksLabOak1StoleParcelText
	text_end
...
.HowIsYourPokedexComingText:
	...
	text_end
	.OakWonText:
	text_far _OaksLabOakWonText
	text_end
	.ChallengeOakText:
	text_far _OaksLabOakChallengeText
	text_end
		.RefusedChallengeText:
	text_far _OaksLabOakRefusedChallengeText
	text_end
OaksLabPokedexText:
	text_asm
	ld hl, .Text
...
OaksLabRivalLeaveItAllToMeText:
	...
	text_end
OaksLabAidePokeballsText:
	text_asm
	ld hl, .Text
	call PrintText
	jp TextScriptEnd
	.Text:
	text_far _OaksLabAidePokeballsText
	text_end
OaksLabAideAfterPokeballsText:
	text_asm
	ld hl, .Text
	call PrintText
	jp TextScriptEnd
		.Text:
	text_far _OaksLabAideAfterPokeballsText
	text_end
...
.Text:
	...
	text_end
OaksLabAideReceivedPokeballsText:
	text_asm
	ld hl, .Text
	call PrintText
	jp TextScriptEnd
.Text:
	text_far _OaksLabAideReceivedPokeballsText
	sound_get_key_item
	text_end
OaksLabOakEndBattleScript:
	...
	xor a
	ld [wJoyIgnore], a
	ld a, SCRIPT_OAKSLAB_POST_OAK_BATTLE
	ld [wOaksLabCurScript], a
	ret
OaksLabOakDiplomaText:
	text_asm
	ld hl, .Text
	call PrintText
	...
	jp TextScriptEnd
	.Text:
	text_far _OaksLabOakDiplomaIntroText
	text_end
OaksLabPostOakBattleScript:
	ld a, TOGGLE_OAKS_LAB_OAK_1
	ld [wToggleableObjectIndex], a
	predef ShowObject
	xor a
