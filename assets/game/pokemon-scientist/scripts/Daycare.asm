...
GiveDayCareBaby:
	...
	; Gives wDayCareBabySpecies at level 5.
	; Caller must ensure there is room in the party.
	ld a, [wDayCareBabySpecies]
	ld [wCurPartySpecies], a
	ld a, 5
	ld [wCurEnemyLevel], a
		; Allow AddPartyMon to display the nickname prompt.
	xor a
...
DaycareGentlemanText:
	...
	ld a, 1
	ld [wDayCareInUse], a
	; reset breeding state for the newly deposited Pokemon
	xor a
	ld [wDayCareBreedSteps], a
	ld [wDayCareBreedSteps + 1], a
	ld [wDayCareBabySpecies], a
	ld a, PARTY_TO_DAYCARE
	ld [wMoveMonType], a
	call MoveMon
	; Determine the baby species from the deposited Pokemon.
	callfar GetDayCareBabySpecies
	xor a
	...
	jp .done
.daycareInUse
	; Check whether the breeding counter has reached 1024 steps ($0400).
	ld a, [wDayCareBreedSteps + 1]
	cp $04
	jr nz, .noBabyReady
	ld a, [wDayCareBreedSteps]
	and a
	jr nz, .noBabyReady
	; A baby is ready.
	ld a, [wDayCareBabySpecies]
...
.enoughMoney
	xor a
	ld [wDayCareInUse], a
	; clear breeding state when the Pokemon leaves daycare
	ld [wDayCareBreedSteps], a
	ld [wDayCareBreedSteps + 1], a
	ld [wDayCareBabySpecies], a
...
.babyReady
	ld a, [wPartyCount]
	cp PARTY_LENGTH
	ld hl, .NoRoomForBabyText
	jr z, .done
		; Announce the baby.
	ld hl, .BabyReadyText
	call PrintText
		; Prepare the species name.
	ld a, [wDayCareBabySpecies]
	ld [wCurPartySpecies], a
	ld [wNamedObjectIndex], a
	call GetMonName
		; Print "<PLAYER> got a ODDISH!" and play the jingle.
	ld hl, .GotBabyText
	call PrintText
		; Add the baby and allow naming.
	ld a, 5
	ld [wCurEnemyLevel], a
	xor a
	ld [wMonDataLocation], a
...
.NotEnoughMoneyText:
	...
	text_end
.BabyReadyText:
	text_far _DaycareGentlemanBabyReadyText
	text_end
.NoRoomForBabyText:
	text_far _DaycareGentlemanNoRoomForBabyText
	text_end
	.GotBabyText:
	text_far _DaycareGentlemanGotBabyText
	sound_get_item_1
	text_end
