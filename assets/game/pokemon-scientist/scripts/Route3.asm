...
Route3SuperNerdText:
	text_asm
	CheckEvent EVENT_GOT_ROUTE3_SQUIRTLE
	jr nz, .After
	ld hl, .IntroText
	call PrintText
	call YesNoChoice
	...
	and a
	jr nz, .Declined
	; Does the player have a Fresh Water?
	ld b, FRESH_WATER
	call IsItemInBag
	jr z, .NoWater
	ld hl, .GiveSquirtleText
	call PrintText
	lb bc, SQUIRTLE, 10
	call GivePokemon
	jr nc, .NoRoom
	...
	SetEvent EVENT_GOT_ROUTE3_SQUIRTLE
	jp TextScriptEnd
.Declined
	ld hl, .DeclinedText
	call PrintText
...
.IntroText
	...
	text_end
.NoWaterText
	text_far _Route3NeedWaterText
	text_end
.DeclinedText
	text_far _Route3DeclinedText
	text_end
.NoRoomText
	text_far _Route3NoRoomText
	text_end
.GiveSquirtleText
	text_far _Route3GiveSquirtleText
	text_end
.AfterText
	text_far _Route3AfterText
	text_end
