...
; Load a new map.
	...
	ld [wJoyIgnore], a
;	Leaving the previous map clears any Flash effect.
	ld hl, wStatusFlags6
...
; step counting
	...
	jr z, .checkCooldown
	ld hl, wMewRoamingSteps
	dec [hl]
	jr nz, .checkCooldown
	ld [hl], $ff
	farcall MoveRoamingMew
.checkCooldown
	ld a, [wStatusFlags2]
...
; function to load data from the map header
	...
	ld a, [wCurMap]
	call SwitchToMapRomBank
	; Initialize cave darkness for the newly loaded map.
	xor a
	ld [wMapPalOffset], a
	ld a, [wCurMap]
	cp VIRIDIAN_FOREST
...
.DarkMap
	ld hl, wStatusFlags6
	bit BIT_FLASH_ACTIVE, [hl]
	jr nz, .AfterDarkCheck
	ld a, $03
	ld [wMapPalOffset], a
.AfterDarkCheck
	ld a, [wCurMapTileset]
