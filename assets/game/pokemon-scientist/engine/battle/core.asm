...
EnemyRan:
	; Roaming Mew that has already been released.
	ld a, [wMewRoamingEncounter]
	and a
	jr z, .checkTruckMew
	; Preserve roaming HP.
	ld a, [wEnemyMonHP]
...
.AIActionUsedEnemyFirst
	...
	ld a, [wMewRoamingEncounter]
	and a
	jr z, .notTruckMewFlee1
.rollMewFlee1
	ld a, [wEnemyMonStatus]
	and (1 << FRZ) | SLP_MASK
	jr nz, .notTruckMewFlee1
	call BattleRandom
	cp 75 percent
	jr nc, .notTruckMewFlee1
	jp EnemyRan
.notTruckMewFlee1
	call HandlePoisonBurnLeechSeed
	jp z, HandlePlayerMonFainted
...
.playerMovesFirst
	...
	ld a, [wMewRoamingEncounter]
	and a
	jr z, .notTruckMewFlee2
.rollMewFlee2
	ld a, [wEnemyMonStatus]
	and (1 << FRZ) | SLP_MASK
	jr nz, .notTruckMewFlee2
	call BattleRandom
	cp 75 percent
	jr nc, .notTruckMewFlee2
	jp EnemyRan
.notTruckMewFlee2
	call HandlePoisonBurnLeechSeed
	jp z, HandlePlayerMonFainted
...
.AIActionUsedPlayerFirst
	...
	ld a, [wMewRoamingEncounter]
	and a
	jp z, .notTruckMewFlee3
.rollMewFlee3
	ld a, [wEnemyMonStatus]
	and (1 << FRZ) | SLP_MASK
	jr nz, .notTruckMewFlee3
	call BattleRandom
	cp 75 percent
	jr nc, .notTruckMewFlee3
	jp EnemyRan
.notTruckMewFlee3
	jp MainInBattleLoop
...
; mark every party member eligible
	ld a, [wPartyCount]
	ld b, 0
.gainExpFlagsLoop
	scf
...
HandlePlayerBlackOut:
    ; Link battles don't blackout.
	ld a, [wLinkState]
	cp LINK_STATE_BATTLING
	jr z, .linkBattle
    ; Starter rival battle?
	ld a, [wCurMap]
	cp OAKS_LAB
	jr nz, .normalBlackout
	ld a, [wCurOpponent]
	cp OPP_RIVAL1
	jr z, .starterLoss
	jr .normalBlackout
.normalBlackout
	...
	call PrintText
    ; Signal the overworld to perform the blackout.
	scf
	ret
.starterLoss
	hlcoord 0, 0
	...
	ld hl, Rival1WinText
	call PrintText
    ; Do NOT blackout after the Oak Lab battle.
	and a        ; clear carry
	ret
.linkBattle
...
.returnAfterCapturingMon
	ld a, [wMewRoamingEncounter]
	and a
	jr z, .notRoamingMewCapture
	ResetEvent EVENT_MEW_ROAMING
.notRoamingMewCapture
	call GBPalNormal
	xor a
...
InitWildBattle:
	ld a, $1
	ld [wIsInBattle], a
	; Re-identify roaming Mew after InitBattleVariables.
	ld a, [wEnemyMonSpecies2]
	cp MEW
	jr nz, .notRoamingMew
